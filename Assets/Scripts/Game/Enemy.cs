...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
         ...
         */
        GameObject nearest_player = GameManager.Instance.NearestPlayer(transform.position);
        if (nearest_player != null){
            Vector3 direction = nearest_player.transform.position - transform.position;
            Quaternion rotation = Quaternion.LookRotation(direction, Vector3.forward);
            mRigidBody.MoveRotation(rotation);
            mRigidBody.MovePosition(transform.position + transform.forward * 3 * Time.fixedDeltaTime);
        }
    }
