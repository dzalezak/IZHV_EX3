...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if(shotgun){
            for(int i = 1; i < (shotgunBullets/2) + 1; i++){
                SpawnBullet(
                    director.position,
                    Quaternion.Euler(director.eulerAngles.x, director.eulerAngles.y, director.eulerAngles.z + (((shotgunSpread*i)/shotgunBullets)+i-3))
                );
                SpawnBullet(
                    director.position,
                    Quaternion.Euler(director.eulerAngles.x, director.eulerAngles.y, director.eulerAngles.z - (((shotgunSpread*i)/shotgunBullets)+i-3))
